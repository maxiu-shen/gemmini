...
static void printMatrix_biger(elem_t *m, size_t m_rows, size_t m_cols) { //shenweilong add, for printing matrices larger than DIM x DIM
  for (size_t i = 0; i < m_rows; ++i) {
    for (size_t j = 0; j < m_cols; ++j) {
      // 核心修正：使用一维偏移计算
      elem_t val = m[i * m_cols + j]; 

#ifndef ELEM_T_IS_FLOAT
      printf("%d ", val);
#else
      printf("%x ", elem_t_to_elem_t_bits(val));
#endif
    }
    printf("\n");
  }
