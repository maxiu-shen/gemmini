...
int main() {
  ...
  gemmini_config_ex(WEIGHT_STATIONARY, 0, 0); // config_ex，WEIGHT_STATIONARY表示权重稳定模式，第一个0表示不使用激活函数
  gemmini_preload(Identity_sp_addr, Out_sp_addr); //将Out_sp_addr地址作为输出矩阵的地址，Identity_sp_addr地址作为权重矩阵的地址加载到脉动阵列中
  gemmini_compute_preloaded(In_sp_addr, GARBAGE_ADDR); //将In_sp_addr地址的矩阵和Identity_sp_addr地址的矩阵进行乘法运算
