...
**⚠️ 编写任何代码前，必须先用 `read_file` 工具加载对应的 instruction 文件，读完确认后才能动手。**

| # | 触发条件 | 必读文件（`read_file` 加载） |
|---|---------|----------------------|
...
Gemmini 是**脉动阵列矩阵乘法加速器**（RISC-V RoCC 协处理器，Chisel 实现，[Chipyard](https://github.com/ucb-bar/chipyard) 生态）。项目包含硬件（Scala/Chisel）和软件（C 裸机测试、Spike 功能模型）两部分。

数据流水线：**CPU → RoCC → LoopConv/LoopMatmul → ReservationStation → Controllers → Scratchpad + Mesh**。
